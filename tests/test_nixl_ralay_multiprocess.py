        ...
        try:
            ...
            while count < num_transfers:
                try:
                    ...
                    metadata = RdmaMetadata(**metadata_obj) if isinstance(metadata_obj, dict) else metadata_obj
                    
                    # Create receive buffer
                    buffer_tensor = torch.empty(item['size'], dtype=item['dtype'], device=device)
                    ...
                    assert torch.allclose(original_values, received_values, rtol=1e-5, atol=1e-5), \
                        f"Data mismatch in transfer {count + 1}"
                    assert not torch.allclose(received_values, torch.zeros_like(received_values)), \
                        f"Received data is all zeros in transfer {count + 1}"
                    
                    count += 1
                    print(f"[Receiver] Transfer {count}/{num_transfers}: {get_time:.2f} ms - PASSED")
...
def test_multiprocess_transfer_with_nixl_ralay():
    ...
    print("=" * 60)
    print(f"Data size: {test_data_size} elements (~{test_data_size * 4 / (1024*1024):.2f} MB)")
